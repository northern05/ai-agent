...
class SMCDriver:
    ...
    def __init__(self,
        ...
        self.bonding_contract_address = bonding_contract_address
        with open('prize_contract.abi', 'r') as file:
            self.prize_contract_abi = json.load(file)

        with open('bonding_curve_contract.abi', 'r') as file:
            self.bonding_contract_abi = json.load(file)
        ...
        self.bonding_contract = self.w3.eth.contract(
            address=Web3.to_checksum_address(self.bonding_contract_address),
            abi=self.bonding_contract_abi,
        )
    ...
    def get_swap_msg_hash(self, transaction_hash):
        try:
            transaction = self.w3.eth.get_transaction(transaction_hash)
            input_data = transaction.get("input")
            if input_data == '0x':
                ...
                print(f"Raw input data: {input_data}")
                # Decode the input data if it follows a specific ABI
                hashedPrompt = self.bonding_contract.decode_function_input(input_data)[1].get("hashedPrompt")
                msg_hash = hashedPrompt.hex()
                print(f"Decoded input data: {msg_hash}")
                return msg_hash
        except Exception as e:
